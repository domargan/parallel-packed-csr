...
void PCSR::double_list() {
    ...
    edges.node_version_counters = (atomic<int> *) realloc(edges.node_version_counters,
...
void PCSR::add_edge_parallel(uint32_t src, uint32_t dest, uint32_t value, int retries) {
    if (value != 0) {
        ...
        pair<pair<int, int>, insertion_info_t *> acquired_locks = acquire_insert_locks(loc_to_add, e, src, bs.second,
                                                                                       -1, 0);
