...
void PCSR::double_list() {
  ...
  edges.node_version_counters = (atomic<int>*)realloc(edges.node_version_counters, (edges.N / edges.logN) * sizeof(atomic_int));
...
pair<uint32_t, int> binary_search(edge_list_t *list, edge_t *elem, uint32_t start,
                                  uint32_t end, shared_timed_mutex** node_locks, bool unlock) {
  int ins_v = -1;
  int index = -1;
...
uint32_t PCSR::find_value(uint32_t src, uint32_t dest) {
  ...
  pair<uint32_t, int> bs =
      binary_search(&edges, &e, nodes[src].beginning + 1, nodes[src].end, node_locks, false);
...
void PCSR::insert(uint32_t index, edge_t elem, uint32_t src, insertion_info_t *info) {
  ...
  if (is_null(edges.items[index].value)) {
    ...
    if (index == edges.N - 1) {
      ...
      uint32_t loc_to_add =
          binary_search(&edges, &elem, node.beginning + 1, node.end, node_locks, false).first;
...
void PCSR::remove_edge(uint32_t src, uint32_t dest) {
  ...
  if (lock_bsearch) {
    ...
    if (nodes[src].beginning != beginning
        || nodes[src].end != end) {
...
PCSR::PCSR(uint32_t init_n, uint32_t src_n, bool lock_search) {
...
bool PCSR::edge_exists(uint32_t src, uint32_t dest) {
  ...
  e.value = 1;
  uint32_t loc_to_rem =
      binary_search(&edges, &e, node.beginning + 1, node.end, node_locks, false).first;
...
// Added by me
uint32_t PCSR::get_node_id(uint32_t node_index) {
  return node_index / edges.logN;
}
...
// Added by me
pair<int, int>
PCSR::acquire_remove_locks(uint32_t index, edge_t elem, uint32_t src, int ins_node_v, uint32_t left_node_bound) {
  int node_index = find_leaf(&edges, index);
  // printf("node_index = %d\n", node_index);
  int level = edges.H;
  int len = edges.logN;
...
// Added by me
PCSR::PCSR(uint32_t init_n, vector<condition_variable*> *cvs, bool lock_search) {
  edges.N = 2 << bsr_word(init_n);
  edges.logN = (1 << bsr_word(bsr_word(edges.N) + 1));
  edges.H = bsr_word(edges.N / edges.logN);
  edges.global_lock = make_shared<shared_timed_mutex>();
...
bool PCSR::got_correct_insertion_index(edge_t ins_edge, uint32_t src, uint32_t index, edge_t elem, int node_index,
  ...
  if (!(is_null(ins_edge.value)) && ((is_sentinel(ins_edge) && src != nodes.size() - 1
                                      && ins_edge.src != src + 1) || (is_sentinel(ins_edge) && src == nodes.size() - 1
                                                                      && ins_edge.value != UINT32_MAX) ||
                                     (!is_sentinel(ins_edge) && ins_edge.src != src))) {
    return false;
  }
  // Check that the current edge is larger than the one we want to insert
  if (!(is_null(ins_edge.value)) && !is_sentinel(ins_edge)
      && ins_edge.dest < elem.dest) {
    return false;
  }
  if (is_null(ins_edge.value)) {
    ...
    if (ind < edges.N && !is_null(item.value) && !is_sentinel(item) && item.src == src
        && item.dest < elem.dest) {
      return false;
    }
    // if it's a sentinel node for the wrong vertex the index is wrong
    if (!(is_null(item.value)) && is_sentinel(item) && ((src != nodes.size() - 1
                                                         && item.value != src + 1) || (src == nodes.size() - 1 && item.value == UINT32_MAX))) {
      return false;
    }
  }
  ...
  if (!is_null(item.value) && !is_sentinel(item)
      && item.src == src && item.dest >= elem.dest) {
    return false;
  }
  if (!is_null(item.value) && is_sentinel(item) && ((src == 0 && item.value != UINT32_MAX)
                                                    || (src != 0 && item.value != src))) {
    return false;
  }
  return true;
}
...
void PCSR::add_edge_parallel(uint32_t src, uint32_t dest, uint32_t value, int retries) {
  if (value != 0) {
    ...
    if (lock_bsearch) {
      ...
      bs =
          binary_search(&edges, &e, beginning + 1, end, node_locks, false);
      loc_to_add = bs.first;
    ...
    pair<pair<int,int>, insertion_info_t*> acquired_locks = acquire_insert_locks(loc_to_add, e, src, bs.second, -1, 0);
    ...
    if (acquired_locks.first.first == NEED_GLOBAL_WRITE) {
      ...
      edges.global_lock->lock();
      loc_to_add =
          binary_search(&edges, &e, nodes[src].beginning + 1, nodes[src].end, node_locks, false).first;
