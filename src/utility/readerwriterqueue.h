 ...
 public:
...
#endif
  {
    ...
    if (largestBlockSize > MAX_BLOCK_SIZE * 2) {
      ...
      // ambiguity between front == tail meaning "empty" and "full". So the effective number of slots that are
      // guaranteed to be usable at any time is the block size - 1 times the number of blocks - 1. Solving for size and
  ...
  // Allocates an additional block of memory if needed.
  // Only fails (returns false) if memory allocation fails.
  AE_FORCEINLINE bool push(T const &element) AE_NO_TSAN { return inner_enqueue<CanAlloc>(element); }

  // Enqueues a moved copy of element on the queue.
  // Allocates an additional block of memory if needed.
  // Only fails (returns false) if memory allocation fails.
  AE_FORCEINLINE bool push(T &&element) AE_NO_TSAN { return inner_enqueue<CanAlloc>(std::forward<T>(element)); }

  inline T front() { return (*(peek())); }

  inline bool empty() const { return (size_approx() == 0); }

  inline size_t size() const { return size_approx(); }
 ...
 private:
  ...
  struct ReentrantGuard {
    AE_NO_TSAN ReentrantGuard(bool &_inSection) : inSection(_inSection) {
      assert(!inSection &&
             "Concurrent (or re-entrant) enqueue or dequeue operation detected (only one thread at a time may hold the "
  ...
  struct Block {
    ...
    char cachelineFiller1[MOODYCAMEL_CACHE_LINE_SIZE - sizeof(weak_atomic<size_t>) -
                          sizeof(size_t)];  // next isn't very contended, but we don't want it on the same cache line as
                                            // tail (which is)
