...
public:
    ...
    size_t size() const noexcept { return was_size(); }

    bool empty() const noexcept { return was_empty(); }

    auto front() noexcept { return pop(); }
};
...
public:
    ...
    AtomicQueueB2(unsigned size)
        ...
        , elements_(AllocatorElements::allocate(size_)) {
      std::cout << "Atomic Q size = " << size << std::endl;
      for (auto p = states_, q = states_ + size_; p < q; ++p) p->store(Base::EMPTY, X);
